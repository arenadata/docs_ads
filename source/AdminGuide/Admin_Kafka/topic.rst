...
**cleanup.policy** -- Определение политики хранения старых сегментов журнала. Политика по умолчанию "delete" отбрасывает старые сегменты при достижении их срока хранения или предельного размера. Значение "compact" включает `сжатие журнала <../../Architecture/compaction>`_ по топику
