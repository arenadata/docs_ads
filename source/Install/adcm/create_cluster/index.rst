...
1. В меню кластера *ads* открыть вкладку "Hosts" (:numref:`Рис.%s.<img_15>`).
...
1. Компоненты сервиса *Zookeeper*:
...
2. Компоненты сервиса *Kafka* (:numref:`Рис.%s.<img_19>`):
...
3. Компоненты сервиса *Nifi*: 

* *nifi.SERVER* -- необходимо добавить на один и более хостов (*df-mdw*);

4. Компоненты сервиса *monitoring clients* (:numref:`Рис.%s.<img_20>`):
...
* Установка кластера. Команда на уровне кластера, представляет собой последовательный вызов процессов установки и конфигурирования всех добавленных сервисов.

* Установка сервиса. Команда на уровне сервиса, представляет собой установку отдельно взятого сервиса.
...
* В поле "Actions" нажать на пиктограмму в строке сервиса *Zookeeper* и выбрать действие *Install* (:numref:`Рис.%s.<img_23>`).
