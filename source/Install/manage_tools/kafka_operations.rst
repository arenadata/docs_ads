...
Когда хосты становятся доступными для подключения по ssh для менеджера кластеров, необходимо выбрать действие *Expand* cервиса *Kafka* из списка возможных операций (см. :numref:`Рис.%s <kafka_action>`). В появившемся диалоговом окне предоставляется выбор следующих опций (:numref:`Рис.%s <expand_config_menu>`):
